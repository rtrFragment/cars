...
int LoadMeshData(char *filename, std::vector<float> &vertices, std::vector<float> &textures, std::vector<float> &normals)
{
	...
	*/
	...
	while (fgets(line, BUFFER_SIZE, g_fp_meshfile) != NULL)
	{
		...
		{
			/*fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf(gpFile, "In LoadMeshData While Loop f Token\n");
			fclose(gpFile);*/
		...
		//	while(strtok_s(token, NULL, &sep_space))
			...
			/*	token_vertex_index = strtok(face_tokens[i], sep_fslash);
				...
				/*strtok_s(token_vertex_index, face_tokens[i], &sep_fslash);
				strtok_s(token_texture_index, NULL, &sep_fslash);
				strtok_s(token_normal_index, NULL, &sep_fslash);*/
		...
		{
			/*fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf(gpFile, "In LoadMeshData Last else loop\n");
			fclose(gpFile);*/
		}
	/*	fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "In LoadMeshData End of While Loop\n");
		fclose(gpFile);*/
	...
	//sprintf(str, "count_of_vertices Value : %d\n count by method : %zu", count_of_vertices,g_temp_vertices.size());
