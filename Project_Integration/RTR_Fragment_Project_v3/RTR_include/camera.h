...
public:
	...
	void MoveCamera(Camera_Movement _Direction, double _unitMoveBy,float delta)
	{
		...
		{
			this->Position.x += (float)_unitMoveBy;
			this->Target.x += (float)_unitMoveBy;
		}
		else if(_Direction == CameraMoveOnY)
		{
			this->Position.y += (float)_unitMoveBy;
			this->Target.y += (float)_unitMoveBy;
		}
		else if (_Direction == CameraMoveOnZ)
		{
			this->Position.z += (float)_unitMoveBy;
			this->Target.z += (float)_unitMoveBy;
		}
		this->updateCameraVectors();
	}
