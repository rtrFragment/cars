...
#include<vector>
#include<stdlib.h>
#include "../RTR_include/Common_Header.h"
	...
	while (fgets(line, BUFFER_SIZE, g_fp_meshfile) != NULL)
	{
		...
		else if (strcmp(first_token, "f") == S_EQUAL)
		{
			...
			for (int i = 0; i != NR_FACE_TOKENS; ++i)
			{
				...
				triangle_vertex_indices[i] = (int)atoi(token_vertex_index);
				texture_vertex_indices[i] = (int)atoi(token_texture_index);
				normal_vertex_indices[i] = (int)atoi(token_normal_index);
...
//	sprintf(str, "count_of_vertices Value : %d\n count by method : %zu", count_of_vertices,g_temp_vertices.size());
	logInfo("count_of_vertices Value : %d\n count by method : %zu", count_of_vertices, g_temp_vertices.size());
