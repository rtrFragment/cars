...
void initialize(void)
{
	...
	fprintf(gpFile, "Init Frequency : %" PRIu64 "\n", initFrequency);
	fprintf(gpFile, "Init Time : %" PRIu64 "\n", initTime);
...
void display(void)
{
	...
	fprintf(gpFile, "Current Time : %f \t Last Time : %f \t Delta Time : %f \n", currentFrame, lastFrame, deltaTime);
	lastFrame = currentFrame;
...
void resize(int width, int height)
{
	...
	gPerspectiveProjectionMatrix = glm::perspective(camera.Zoom, (GLfloat)width / (GLfloat)height, 0.1f, 100.0f);
}

float getTime(void)
{
	QueryPerformanceCounter((LARGE_INTEGER*)&changingTime);
	uint64_t time = (changingTime - initTime) / initFrequency;
	return ((float)time);
}
